...
//		testUpdateOne();
	...
	private static void testUpdateMany() {
		...
		// 실제 문서가 매칭이되어도 내용이 다르지 않으면 변경되지 않는다.
		System.out.println(result.getMatchedCount() + "개의 문서를 찾았음!");
	...
	private static void testUpdateOne() {
		...
		// 주의 : 업데이트 시 $set 연산잘르 사용해야 update
...
//		Bson bsonFilter = Filters.and(Filters.eq("species", "인간"), Filters.eq("gender", "FEMALE"));
	...
	private static void testInsertMany() {
		// db.javaMongo.insertMany([ {문서1}, {문서2} ]};
		...
		doc = new Document("name", "고영희").append("species", "인간").append("gender", "FEMALE");
		docs.add(doc);
