...
public class AuthorDAOImpl implements AuthorDAO {
	...
	private Connection getConnection() throws SQLException {
		...
		try {
			...
			conn = DriverManager.getConnection(dburl, "c##kimjh", "1234");
...
//			rs.next(); // 첫번째 레코드?
	...
	public boolean insert(AuthorVO vo) {
		// .executeUpdate 메서드 사용 -> int(삽입된 레코드 수)반환
		Connection conn = null;
		PreparedStatement pstmt = null;
		int insertdCount = 0;
		...
		try {
			...
			// 쿼리 수행
			insertdCount = pstmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			try {
		...
		return 1 == insertdCount; // 삽입된 레코드가 1개이면 성공
	}
